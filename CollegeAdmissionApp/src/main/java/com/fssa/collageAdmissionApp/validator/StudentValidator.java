...
public class StudentValidator {
	...
	public static boolean validateStudent(Student student) throws InvalidStudentException, InvalidEmailException, InvalidStudentDobException, InvaliMobileNumberException {
		if (student == null) {
	...
	public static boolean validateEmail(String emailId) throws InvalidEmailException {
		String regex = "^[A-Za-z0-9+_.-]+@(.+)$";
		boolean matches = Pattern.compile(regex).matcher(emailId).matches();
		if (matches) {
			return true;
		} else {
			throw new InvalidEmailException(StudentsErrors.INVALID_EMAIL);
		}
	}
	...
	public static boolean validateDateOfBirth(LocalDate dob) throws InvalidStudentDobException {
        ...
        if (years < 17) {
        	throw new InvalidStudentDobException(StudentsErrors.INVALID_DOB);

        }
        return true;
	}

	public static boolean validateMobileNumber(String mobileNumber) throws InvaliMobileNumberException {
		String regex = "^[6789]\\d{9}$";
		boolean matches = Pattern.compile(regex).matcher(mobileNumber).matches();
		if (matches) {
			return true;
		} else {
			throw new InvaliMobileNumberException(StudentsErrors.INVALID_MOBILE_NUMBER);
		}
	}
